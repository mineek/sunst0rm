...
def prep_restore(ipsw, blob, board, kpp):
    ...
    subprocess.run(['/usr/local/bin/pyimg4', 'im4p', 'create', '-i', './work/ramdisk.dmg', '-o', './work/ramdisk.im4p', '-f', 'rdsk'])
    ...
    if kpp:
        subprocess.run(['/usr/local/bin/pyimg4', 'im4p', 'extract', '-i', './work/' + kernelcache, '-o', './work/kcache.raw', '--extra', './work/kpp.bin'])
    else:
        subprocess.run(['/usr/local/bin/pyimg4', 'im4p', 'extract', '-i', './work/' + kernelcache, '-o', './work/kcache.raw'])
    ...
    if kpp:
        subprocess.run(['/usr/local/bin/pyimg4', 'im4p', 'create', '-i', './work/krnl.patched', '-o', './work/krnl.im4p', '--extra', './work/kpp.bin', '-f', 'rkrn', '--lzss'])
    else:
        subprocess.run(['/usr/local/bin/pyimg4', 'im4p', 'create', '-i', './work/krnl.patched', '-o', './work/krnl.im4p', '-f', 'rkrn', '--lzss'])
    # done!
    print('[*] Done!')
...
def prep_boot(ipsw, blob, board, kpp, identifier, legacy):
    ...
    if kpp:
        subprocess.run(['/usr/local/bin/pyimg4', 'im4p', 'create', '-i', './work/krnlboot.patched', '-o', './work/krnlboot.im4p', '--extra', './work/kpp.bin', '-f', 'rkrn', '--lzss'])
    else:
        subprocess.run(['/usr/local/bin/pyimg4', 'im4p', 'create', '-i', './work/krnlboot.patched', '-o', './work/krnlboot.im4p', '-f', 'rkrn', '--lzss'])
    # sign it like this:  pyimg4 img4 create -p krnlboot.im4p -o krnlboot.img4 -m IM4M
    print('[*] Signing Kernel')
    subprocess.run(['/usr/local/bin/pyimg4', 'img4', 'create', '-p', './work/krnlboot.im4p', '-o', './work/krnlboot.img4', '-m', 'IM4M'])
