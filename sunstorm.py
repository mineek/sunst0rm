...
def prep_restore(ipsw, blob, board, kpp, legacy, skip_baseband):
    ...
    if input() == 'y':
        ...
        if input() == 'y':
            # restore the device using futurestore like this: futurerestore -t blob --use-pwndfu --skip-blob --rdsk ramdisk.im4p --rkrn krnl.im4p -i -g 19H12 --latest-sep --latest-baseband ipsw.ipsw
            print('[*] Restoring Device')
            if skip_baseband:
                subprocess.run(['/usr/local/bin/futurerestore', '-t', blob, '--use-pwndfu', '--skip-blob', '--rdsk',
                               'work/ramdisk.im4p', '--rkrn', 'work/krnl.im4p', '-i', '-g 19H12', '--latest-sep', '--no-baseband', ipsw])
            else:
                subprocess.run(['/usr/local/bin/futurerestore', '-t', blob, '--use-pwndfu', '--skip-blob', '--rdsk',
                               'work/ramdisk.im4p', '--rkrn', 'work/krnl.im4p', '-i', '-g 19H12', '--latest-sep', '--latest-baseband', ipsw])
            # exit
            print('[*] Done!')
            # clean
            print('[*] Cleaning')
            ...
            if skip_baseband:
                print('[!] You can restore the device later using futurestore like this: futurerestore -t blob --use-pwndfu --skip-blob --rdsk work/ramdisk.im4p --rkrn work/krnl.im4p -i -g 19H12 --latest-sep --no-baseband ipsw.ipsw')
            else:
                print('[!] You can restore the device later using futurestore like this: futurerestore -t blob --use-pwndfu --skip-blob --rdsk work/ramdisk.im4p --rkrn work/krnl.im4p -i -g 192H12 --latest-sep --latest-baseband ipsw.ipsw')
            # exit
            sys.exit(0)
    else:
        print('[*] Exiting')
